...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const logoMask = document.querySelector('#logoRevealMask path') as SVGPathElement;

  const initialOverlayScale = 3440;

  logoMask?.setAttribute("d", logoData)

  const logoDimensions = logoContainer?.getBoundingClientRect() ?? { width: 0, height: 0, left: 0, top: 0 };
  const logoBoundingBox = logoMask?.getBBox();

  const horizontalScaleRatio = logoDimensions?.width / logoBoundingBox?.width;

  const vetrticalScaleRatio = logoDimensions?.height / logoBoundingBox?.height;

  const logoScaleFactor = Math.min(horizontalScaleRatio, vetrticalScaleRatio);
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      if (scrollProgress >= 0.6 && scrollProgress <= 0.85) {
        const overlayCopyRevealProgress = (scrollProgress - 0.6) / (0.25);
